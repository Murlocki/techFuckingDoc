...
class Lab1Controller:
    ...
    def start_calc(self):
        ...
        try:
            ...
            self.window.updatePoint([[self.x_start, self.y_start,self.function.get_function_point(self.x_start, self.y_start)]], color='red',delay=delay)
            arrowHeight = self.window.graph.axes.get_zlim()[1] / 10
            self.window.graph.draw_arrow(self.x_start, self.y_start,self.function.get_function_point(self.x_start, self.y_start) + arrowHeight,0, 0, -arrowHeight)
